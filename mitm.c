...
int golden_claw_search(int maxres, u64 k1[], u64 k2[])
{
    double start = wtime();
    u64 N = 1ull << n;
    //modifier la boucle for pour que chaque process fais 2^n/p itérations
    for (u64 x = rank*N/size; x < (rank+1)*N/size; x++) {
        //Calcul d'une valeur
        u64 z = f(x);
        u64 key_values[N/size];
        int position=0;
        //reste de la div euclidienne de z par p
        //Selon le resultat, on envoie la la valeur et la clé associé à la partie du dictionnaire associé au reste de la div euclidienne obtenu
        //Send et Recv ?
        if ( z%p == rank){
            dict_insert(z, x);
        }
            ...
            //buffer
            key_values[position]=(z,x);// à vérifier
            position++;
        }
    }
    //échange de la taille des buffers
    int buffer_recv_position[size];
    int taille_max=0;
    MPI_Alltoall(position, 1, MPI_INT, buffer_recv_position, 1, MPI_INT, MPI_COMM_WORLD);
    for (int i=0;i<size;i++){
        taille_max+= buffer_recv_position[i];
    }

    //envoi des clé_valeurs
    u64* buffer_recv[size];
    MPI_Alltoall(&key_values,N/size, MPI_UINT64, buffer_recv,N/size, MPI_UINT64, MPI_COMM_WORLD);

    u64 liste_finale[taille_max];
    int index=0;

    //création de la liste de taille max que l'on va parcourir
    for (int i=0;i<size-1;i++){
        for (int j=0;j<buffer_recv_position[i];j++){
            liste_finale[j+index]=buffer_recv[i][j];
        }
        index+=buffer_recv_position[i];
    }
    
    //parcourir la liste finale avec toutes les clés valeurs et voir si elles sont dans ce dictionnaire (celui du processus courant)
    for (int i=0;i<taille_max;i++){
        if ( (liste_finale[i][0] % size ) == rank){
            dict_insert(liste_finale[i][0],liste_finale[i][1]);
        }
    }
    //fin dict_insert
...
void process_command_line_options(int argc, char ** argv)
{
        ...
        while ((ch = getopt_long(argc, argv, "", longopts, NULL)) != -1) {
                ...
                case '1':
                        ...
                        C[1][1] = c1 >> 32;
                        break;
                default:./mitm --n 15 --C0 f57541a84d3cc0e6 --C1 a286b2c9a185784f
        }
        if (n == 0 || set != 3) {
        	usage(argv);
        	exit(1);
